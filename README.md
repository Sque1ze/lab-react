...
### 🔍 Опис
1. App — це “корінь” програми, звідси все починається.

2. TodoList отримує дані через useTodos, і розподіляє їх між підкомпонентами.

3. AddTodoForm викликає onAdd() → додає нове завдання.

4. TodoItem викликає onToggle() або onDelete() → змінює або видаляє задачу.

5. useTodos — виконує запити до Fake REST API (DummyJSON).

6. Дані повертаються назад у TodoList, і React оновлює інтерфейс.
